...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[], skip=False, opt=None):
    ...
    elif which_model_netG == 'orcgan_unet_resize':
        netG = ORCGAN_Unet_resize_conv(opt, skip)
    elif which_model_netG == 'orcgan_unet_test_resize':
        netG = ORCGAN_Unet_test_resize_conv(opt, skip)
    else:
...
def define_D(input_nc, ndf, which_model_netD,
             n_layers_D=3, norm='batch', use_sigmoid=False,use_constraint=False, gpu_ids=[], patch=False):
    netD = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type=norm)
    ...
    if which_model_netD == 'basic':
        ...
        netD = NoNormDiscriminator(input_nc, ndf, n_layers_D, use_sigmoid=use_sigmoid, use_constraint=use_constraint, gpu_ids=gpu_ids)
...
class NoNormDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, use_sigmoid=False, use_constraint=False, gpu_ids=[]):
        ...
        #     sequenceB += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]

        #     if use_sigmoid:
        #         sequenceB += [nn.Sigmoid()]

        #     self.model_A = nn.Sequential(*sequenceA)
        #     self.model_B = nn.Sequential(*sequenceB)
    ...
    def forward(self, input):
        ...
        #     DA = self.model_A(input)
        #     # print("D_forward inputsize:" + str(input.size()))
        #     b, c, h, w = input.size()
        #     constraint_ = torch.ones(torch.Size([b,2,int(h/2)+1,int(w/2)+1]))
            
        #     for bx in range(0,b):
        #         constraint_[bx,0,:,:] = condi_mean[bx]+0.000001
        #         constraint_[bx,1,:,:] = condi_std[bx]+0.000001
        #         # print("mean"+str(condi_mean[bx])+",std"+str(condi_std[bx]))
        #     # coor_ = ((hx/h)**2 + (wx/w)**2)**0.5
        #     # constraint_[bx,0,hx,wx] =  math.exp( -((coor_-condi_mean[bx])**2/(condi_std[bx]**2)*2) )/ (condi_std[bx]*2.50663) #root(2pi)=2.50663
            
        #     constraint_ = constraint_.cuda()

            
        #     con_input = torch.cat((DA,constraint_),1)
        #     D = self.model_B(con_input)
...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        ...
        if opt.self_attention:
            self.conv1_1 = nn.Conv2d(5, 32, 3, padding=p)
        ...
        self.conv2_1 = nn.Conv2d(33, 64, 3, padding=p)
        ...
        self.conv3_1 = nn.Conv2d(65, 128, 3, padding=p)
        ...
        self.conv4_1 = nn.Conv2d(129, 256, 3, padding=p)
        ...
        self.conv5_1 = nn.Conv2d(257, 512, 3, padding=p)
    ...
    def forward(self, input, gray, constraint):
        flag = 0
        ...
        constraint, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(constraint)
        ...
        if self.opt.self_attention:
            ...
            constraint_2 = self.downsample_1(constraint)
            constraint_3 = self.downsample_2(constraint_2)
            constraint_4 = self.downsample_3(constraint_3)
            constraint_5 = self.downsample_4(constraint_4)

        if self.opt.use_norm == 1:
            if self.opt.self_attention:
                att_ = torch.cat((input, gray), 1)
                con_ = torch.cat((att_, constraint), 1)
                x = self.bn1_1(self.LReLU1_1(self.conv1_1(con_)))
            ...
            x = torch.cat((x, constraint_2), 1)
            x = self.bn2_1(self.LReLU2_1(self.conv2_1(x)))
            conv2 = self.bn2_2(self.LReLU2_2(self.conv2_2(x)))
            x = self.max_pool2(conv2)

            x = torch.cat((x, constraint_3), 1)
            x = self.bn3_1(self.LReLU3_1(self.conv3_1(x)))
            conv3 = self.bn3_2(self.LReLU3_2(self.conv3_2(x)))
            x = self.max_pool3(conv3)
            
            x = torch.cat((x, constraint_4), 1)
            x = self.bn4_1(self.LReLU4_1(self.conv4_1(x)))
            conv4 = self.bn4_2(self.LReLU4_2(self.conv4_2(x)))
            x = self.max_pool4(conv4)

            x = torch.cat((x, constraint_5), 1)
            x = self.bn5_1(self.LReLU5_1(self.conv5_1(x)))
            x = x*gray_5 if self.opt.self_attention else x
            # x = torch.cat((x*gray_5,constraint_5), 1) if self.opt.self_attention else x
            ...
            # conv4 = torch.cat((conv4*gray_4,constraint_4), 1) if self.opt.self_attention else conv4
            ...
            # conv3 = torch.cat((conv3*gray_3,constraint_3), 1) if self.opt.self_attention else conv3
            ...
            # conv2 = torch.cat((conv2*gray_2,constraint_2), 1) if self.opt.self_attention else conv2
...
class ORCGAN_Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        super(ORCGAN_Unet_resize_conv, self).__init__()

        self.opt = opt
        self.skip = skip
        p = 1

        #---------------Content Encoder Layer Define---------------Start
        if opt.self_attention:
            self.conv1_1 = nn.Conv2d(3, 32, 3, padding=p)
            self.downsample_1 = nn.MaxPool2d(2)
            self.downsample_2 = nn.MaxPool2d(2)
        ...
        #---------------Content Encoder Layer Define---------------End


        #-------------------- Decoder Layer Define --------------------Start
        self.deconv5 = nn.Conv2d(512, 256, 3, padding=p)
        self.conv6_1 = nn.Conv2d(512, 256, 3, padding=p)
        self.LReLU6_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
    ...
    def forward(self, inputA, grayA, UA, VA, Thr):

        flag = 0
        batch_size = inputA.size()[0]

        if inputA.size()[3] > 2200:
            avg = nn.AvgPool2d(2)
            inputA = avg(inputA)
            grayA = avg(grayA)
            flag = 1
        ...
        inputA, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(inputA)
        grayA, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(grayA)
        UA, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(UA)
        VA, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(VA)


        


        c_color_A = torch.cat((UA, VA), 1)

        if self.opt.self_attention:
            grayA_2 = self.downsample_1(grayA)
            grayA_3 = self.downsample_2(grayA_2)
            grayA_4 = self.downsample_3(grayA_3)
            grayA_5 = self.downsample_4(grayA_4)

        if self.opt.use_norm == 1:
            
            #------------------------Content Encoder A-------------------------Start
            
            c_att_A = torch.cat((grayA ,c_color_A), 1)
            c_EnA_x1 = self.bn1_1(self.LReLU1_1(self.conv1_1(c_att_A)))
            c_EnA_conv1 = self.bn1_2(self.LReLU1_2(self.conv1_2(c_EnA_x1)))
            c_EnA_x = self.max_pool1(c_EnA_conv1)


            c_EnA_x = self.bn2_1(self.LReLU2_1(self.conv2_1(c_EnA_x)))
            c_EnA_conv2 = self.bn2_2(self.LReLU2_2(self.conv2_2(c_EnA_x)))
            c_EnA_x = self.max_pool2(c_EnA_conv2)

            c_EnA_x = self.bn3_1(self.LReLU3_1(self.conv3_1(c_EnA_x)))
            c_EnA_conv3 = self.bn3_2(self.LReLU3_2(self.conv3_2(c_EnA_x)))
            c_EnA_x = self.max_pool3(c_EnA_conv3)

            c_EnA_x = self.bn4_1(self.LReLU4_1(self.conv4_1(c_EnA_x)))
            c_EnA_conv4 = self.bn4_2(self.LReLU4_2(self.conv4_2(c_EnA_x)))
            c_EnA_x = self.max_pool4(c_EnA_conv4)

            c_EnA_x = self.bn5_1(self.LReLU5_1(self.conv5_1(c_EnA_x)))
            c_EnA_x = c_EnA_x*grayA_5 if self.opt.self_attention else c_EnA_x

            ContentA = self.bn5_2(self.LReLU5_2(self.conv5_2(c_EnA_x)))
            #------------------------Content Encoder A-------------------------End
            

            #--------------------------Decoder AB---------------------------Start
            DeAB_conv5 = F.upsample(ContentA, scale_factor=2, mode='bilinear')
            c_EnA_conv4 = c_EnA_conv4*grayA_4 if self.opt.self_attention else c_EnA_conv4
            #--------------------------------------------------------------------            
            DeAB_up6 = torch.cat([self.deconv5(DeAB_conv5), c_EnA_conv4], 1)
            xAB = self.bn6_1(self.LReLU6_1(self.conv6_1(DeAB_up6)))
            DeAB_conv6 = self.bn6_2(self.LReLU6_2(self.conv6_2(xAB)))
            DeAB_conv6 = F.upsample(DeAB_conv6, scale_factor=2, mode='bilinear')
            c_EnA_conv3 = c_EnA_conv3*grayA_3 if self.opt.self_attention else c_EnA_conv3
            #--------------------------------------------------------------------
            DeAB_up7 = torch.cat([self.deconv6(DeAB_conv6), c_EnA_conv3], 1)
            xAB = self.bn7_1(self.LReLU7_1(self.conv7_1(DeAB_up7)))
            DeAB_conv7 = self.bn7_2(self.LReLU7_2(self.conv7_2(xAB)))
            DeAB_conv7 = F.upsample(DeAB_conv7, scale_factor=2, mode='bilinear')
            c_EnA_conv2 = c_EnA_conv2*grayA_2 if self.opt.self_attention else c_EnA_conv2
            #--------------------------------------------------------------------
            DeAB_up8 = torch.cat([self.deconv7(DeAB_conv7), c_EnA_conv2], 1)
            xAB = self.bn8_1(self.LReLU8_1(self.conv8_1(DeAB_up8)))
            DeAB_conv8 = self.bn8_2(self.LReLU8_2(self.conv8_2(xAB)))

            DeAB_conv8 = F.upsample(DeAB_conv8, scale_factor=2, mode='bilinear')
            c_EnA_conv1 = c_EnA_conv1*grayA if self.opt.self_attention else c_EnA_conv1
            #--------------------------------------------------------------------
            DeAB_up9 = torch.cat([self.deconv8(DeAB_conv8), c_EnA_conv1], 1)
            xAB = self.bn9_1(self.LReLU9_1(self.conv9_1(DeAB_up9)))
            DeAB_conv9 = self.LReLU9_2(self.conv9_2(xAB))

            DeAB_latent = self.conv10(DeAB_conv9)

            if self.opt.times_residual:
                DeAB_latent = DeAB_latent*grayA

            if self.opt.tanh:
                DeAB_latent = self.tanh(DeAB_latent)
            if self.skip:
                
                if self.opt.linear_add:
                    if self.opt.latent_threshold:
                        DeAB_latent = F.relu(DeAB_latent)
                    elif self.opt.latent_norm:
                        DeAB_latent = (DeAB_latent - torch.min(DeAB_latent))/(torch.max(DeAB_latent)-torch.min(DeAB_latent))
                    # inputA = (inputA - torch.min(inputA))/(torch.max(inputA) - torch.min(inputA))
                    # DeAB_output = DeAB_latent + inputA*self.opt.skip
                    # DeAB_output = DeAB_output*2 - 1
                    
                    if self.opt.self_attention:

                        c_att_A = (c_att_A - torch.min(c_att_A))/(torch.max(c_att_A) - torch.min(c_att_A))
                        


                    if self.opt.power_constraint:
                        DeAB_Y = DeAB_latent[:,0] 
                        DeAB_latent[:,0] = torch.where(DeAB_Y > 0, torch.full_like(DeAB_Y, 0), DeAB_Y)


                    DeAB_output = DeAB_latent + c_att_A*self.opt.skip

                    DeAB_output = torch.where(DeAB_output > 1, torch.full_like(DeAB_output, 1), DeAB_output)
                    DeAB_output = torch.where(DeAB_output < 0, torch.full_like(DeAB_output, 0), DeAB_output)
                    

                    # r = DeAB_output[:,0] + (1.13983 * (DeAB_output[:,2]-0.5))
                    # r = torch.unsqueeze(r, 1)
                    
                    # g = DeAB_output[:,0] - (0.39465 * (DeAB_output[:,1]-0.5)) - (0.58060 * (DeAB_output[:,2]-0.5))
                    # g = torch.unsqueeze(g, 1)
                    
                    # b = DeAB_output[:,0] + (2.03211 * (DeAB_output[:,1]-0.5))
                    # b = torch.unsqueeze(b, 1)

                    v,h,s = DeAB_output[:,0],DeAB_output[:,1],DeAB_output[:,2]
                    
                    h = h*360

                    cc = torch.mul(v,s)
                    div_h = 1-torch.abs((h/60)%2-1)
                    xx = torch.mul(cc,div_h)
                    mm = torch.sub(v,cc)

                    R_ = torch.where(((h>=0)&(h<60))|(((h>=300)&(h<=360))),cc,torch.full_like(cc, 0))
                    R_ = torch.where(((h>=60)&(h<120))|(((h>=240)&(h<300))),xx,R_)
                    G_ = torch.where((h>=60)&(h<180),cc,torch.full_like(cc, 0))
                    G_ = torch.where(((h>=0)&(h<60))|(((h>=180)&(h<240))),xx,G_)
                    B_ = torch.where((h>=180)&(h<300),cc,torch.full_like(cc, 0))
                    B_ = torch.where(((h>=120)&(h<180))|(((h>=300)&(h<=360))),xx,B_)
                    ...
                    DeAB_output = torch.cat((r, g), 1)
                    DeAB_output = torch.cat((DeAB_output, b), 1)
                    DeAB_output = DeAB_output*2 - 1
                    DeAB_output = torch.where(DeAB_output > 1, torch.full_like(DeAB_output, 1), DeAB_output)
                    DeAB_output = torch.where(DeAB_output < -1, torch.full_like(DeAB_output, -1), DeAB_output)
                    # print("output_max:"+str(torch.max(DeAB_output)))
                    # print("output_min:"+str(torch.min(DeAB_output)))
                else:
                    if self.opt.latent_threshold:
                        DeAB_latent = F.relu(DeAB_latent)
                    elif self.opt.latent_norm:
                ...
                DeAB_output = DeAB_latent

            # print("inputA"+str(inputA))
            # print("grayA"+str(grayA))
            # print("c_att_A"+str(c_att_A))
            # print("c_EnA_x"+str(c_EnA_x))
            # print("c_EnA_conv1"+str(c_EnA_conv1))
            # print("c_EnA_conv2"+str(c_EnA_conv2))
            # print("c_EnA_conv3"+str(c_EnA_conv3))
            # print("c_EnA_conv4"+str(c_EnA_conv4))
            # print("ContentA"+str(ContentA))
            # print("CodeAB"+str(CodeAB))
            # print("DeAB_conv5"+str(DeAB_conv5))
            # print("DeAB_conv6"+str(DeAB_conv6))
            # print("DeAB_conv7"+str(DeAB_conv7))
            # print("DeAB_conv8"+str(DeAB_conv8))
            # print("DeAB_conv9"+str(DeAB_conv9))
            # print("DeAB_output"+str(DeAB_output))

            if self.opt.linear:
                DeAB_output = DeAB_output/torch.max(torch.abs(DeAB_output))
            #--------------------------Decoder AB---------------------------End

            r,g,b = DeAB_output[:,0]+1, DeAB_output[:,1]+1, DeAB_output[:,2]+1
            DeAB_output_gray = (0.299*r+0.587*g+0.114*b)/2.
            DeAB_output_gray = DeAB_output_gray[:,np.newaxis,:,:]
        
        DeAB_output = pad_tensor_back(DeAB_output, pad_left, pad_right, pad_top, pad_bottom)
        DeAB_latent = pad_tensor_back(DeAB_latent, pad_left, pad_right, pad_top, pad_bottom)
        
        grayA = pad_tensor_back(grayA, pad_left, pad_right, pad_top, pad_bottom)
        
        if flag == 1:

            DeAB_output = F.upsample(DeAB_output, scale_factor=2, mode='bilinear')

            grayA = F.upsample(grayA, scale_factor=2, mode='bilinear')
        if self.skip:
            
            return DeAB_output , DeAB_latent , grayA
        else:
            return DeAA_output



class ORCGAN_Unet_test_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        super(ORCGAN_Unet_test_resize_conv, self).__init__()
    ...
    def forward(self, inputA, grayA, UA, VA, Thr):
        ...
        # grayA = grayA - Thr
        # grayA = torch.where(grayA < 0, torch.full_like(grayA, 0), grayA)
        ...
        grayA = pad_tensor_back(VA, pad_left, pad_right, pad_top, pad_bottom)
        # print(grayA.size())
        # m_out = YYY
        # m_out -= torch.min(DeAB_latent)
        # m_out /= torch.max(DeAB_latent)
        # DeAB = torch.unsqueeze(m_out, 1)
        # print(DeAB.size())
