...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.input_A_constraint = self.Tensor(nb, 1, size, size)
        self.input_B_constraint = self.Tensor(nb, 1, size, size)
    ...
    def set_input(self, input):
        ...
        b , c , h , w = input_A_gray.size()
        input_A_constraint = torch.zeros((b,c,h,w))
        input_B_constraint = torch.zeros((b,c,h,w))
        for bx in range(0,b):
            
            

            r,g,b = input_B[bx,0]+1, input_B[bx,1]+1, input_B[bx,2]+1
            B_gray = (0.299*r+0.587*g+0.114*b)/2.
            a_constraint = 1#torch.mean(B_gray)
            b_constraint = torch.mean(B_gray)
            input_A_constraint[bx,:,:,:] = a_constraint
            input_B_constraint[bx,:,:,:] = b_constraint


        self.input_A.resize_(input_A.size()).copy_(input_A)
        self.input_A_gray.resize_(input_A_gray.size()).copy_(input_A_gray)
        self.input_A_constraint.resize_(input_A_constraint.size()).copy_(input_A_constraint)
        self.input_B.resize_(input_B.size()).copy_(input_B)
        self.input_B_constraint.resize_(input_B_constraint.size()).copy_(input_B_constraint)
    ...
    def predict(self):
        self.real_A = Variable(self.input_A, volatile=True)
        self.real_A_gray = Variable(self.input_A_gray, volatile=True)
        self.real_A_constraint = Variable(self.input_A_constraint, volatile=True)
        ...
        # print(np.transpose(self.real_A.data[0].cpu().float().numpy(),(1,2,0))[:2][:2][:])
        if self.opt.skip == 1:
            self.fake_B, self.latent_real_A = self.netG_A.forward(self.real_A, self.real_A_gray, self.real_A_constraint)
        else:
    ...
    def backward_D_A_basic(self, netD, real, fake, use_ragan):
        ...
        pred_real = netD.forward(real,self.real_B_constraint)
        pred_fake = netD.forward(m_fake,self.real_A_constraint)
        ...
        b, c, h, w = m_fake.size()
        loss_D_constraint_fake = 0.
        for bx in range(0,b):
            fake_r,fake_g,fake_b = m_fake[bx,0]+1, m_fake[bx,1]+1, m_fake[bx,2]+1
            fake_gray = (0.299*fake_r+0.587*fake_g+0.114*fake_b)/2.
            input_constraint = self.real_A_constraint[bx,0,0,0]
            loss_D_constraint_fake += torch.pow((torch.mean(fake_gray) - input_constraint),2)/b


        loss_D = (loss_D + loss_D_constraint_fake) * 0.5
    ...
    def backward_D_P_basic(self, netD, real, fake, use_ragan):
        # Real
        pred_real = netD.forward(real,real)
        pred_fake = netD.forward(fake.detach(),fake.detach())
        if self.opt.use_wgan:
            loss_D_real = pred_real.mean()
            loss_D_fake = pred_fake.mean()
            loss_D = loss_D_fake - loss_D_real + self.criterionGAN.calc_gradient_penalty(netD, 
    ...
    def forward(self):
        ...
        self.real_A_constraint = Variable(self.input_A_constraint)
        self.real_B_constraint = Variable(self.input_B_constraint)
        ...
        if self.opt.input_linear:
            self.real_A = (self.real_A - torch.min(self.real_A))/(torch.max(self.real_A) - torch.min(self.real_A))
        if self.opt.skip == 1:
            self.fake_B, self.latent_real_A = self.netG_A.forward(self.real_img, self.real_A_gray, self.real_A_constraint)
        else:
            self.fake_B = self.netG_A.forward(self.real_img, self.real_A_gray, self.real_A_constraint)
        if self.opt.patchD:
    ...
    def backward_G(self, epoch):
        ...
        pred_fake = self.netD_A.forward(self.fake_B,self.real_A_constraint)
        if self.opt.use_wgan:
            self.loss_G_A = -pred_fake.mean()
        elif self.opt.use_ragan:
            pred_real = self.netD_A.forward(self.real_B,self.real_B_constraint)
        ...
        m_fake = self.fake_B.detach()
        b, c, h, w = m_fake.size()
        loss_G_constraint_fake = 0.
        for bx in range(0,b):
            fake_r,fake_g,fake_b = m_fake[bx,0]+1, m_fake[bx,1]+1, m_fake[bx,2]+1
            fake_gray = (0.299*fake_r+0.587*fake_g+0.114*fake_b)/2.
            input_constraint = self.real_A_constraint[bx,0,0,0]
            loss_G_constraint_fake += torch.pow((torch.mean(fake_gray) - input_constraint),2)/b

        self.loss_G_A += loss_G_constraint_fake


        loss_G_A = 0
        if self.opt.patchD:
            pred_fake_patch = self.netD_P.forward(self.fake_patch,self.fake_patch)
            if self.opt.hybrid_loss:
                loss_G_A += self.criterionGAN(pred_fake_patch, True)
            else:
                pred_real_patch = self.netD_P.forward(self.real_patch,self.real_patch)
        ...
        if self.opt.patchD_3 > 0:   
            for i in range(self.opt.patchD_3):
                pred_fake_patch_1 = self.netD_P.forward(self.fake_patch_1[i],self.fake_patch_1[i])
                if self.opt.hybrid_loss:
                    loss_G_A += self.criterionGAN(pred_fake_patch_1, True)
                else:
                    pred_real_patch_1 = self.netD_P.forward(self.real_patch_1[i],self.real_patch_1[i])
